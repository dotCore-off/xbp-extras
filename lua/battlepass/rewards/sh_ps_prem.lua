...
function REWARD:GetTooltip(reward)
	...
	return string.Comma(reward) .. " premium point" .. (reward != 1 and "s" or "")
end
...
function REWARD:Unlock(ply, reward, amount)
    ...
    // Give the configured amount of premium points to player
    return ply:SH_AddPremiumPoints(reward)
end

// Reward registration
REWARD:Register("sh_ps_prem")